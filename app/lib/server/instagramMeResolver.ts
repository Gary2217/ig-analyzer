...
function pickProfile(raw: any): {
  ...
  return {
    ...
    followers_count: typeof p?.followers_count === "number" || typeof p?.followers_count === "string" ? p.followers_count : null,
    follows_count: typeof p?.follows_count === "number" || typeof p?.follows_count === "string" ? p.follows_count : null,
    media_count: typeof p?.media_count === "number" || typeof p?.media_count === "string" ? p.media_count : null,
  }
}

function getIsHttps(req: Request, h: Headers) {
  const xfProto = h.get("x-forwarded-proto")?.toLowerCase()
  try {
    const protocol = new URL(req.url).protocol
    return xfProto === "https" || protocol === "https:"
  } catch {
    return xfProto === "https"
  }
}

export async function getMeState(req: Request) {
  try {
    ...
    if (hasToken && (!pageId || !igId)) {
      try {
        const graphUrl = new URL(`https://graph.facebook.com/${INSTAGRAM_GRAPH_VERSION}/me/accounts`)
    ...
    if (hasToken && hasIds) {
      const graphBase = `https://graph.facebook.com/${INSTAGRAM_GRAPH_VERSION}`
      ...
      try {
        ...
        igProfileUrl.searchParams.set("fields", "id,username,name,profile_picture_url,followers_count,follows_count,media_count")
    ...
    return {
      connected: Boolean(hasToken && hasIds),
