...
export default function DemoToolPanel({ activeLocale, isConnectedFromServer, checking }: Props) {
  ...
  function onAnalyzeAccount() {
    if (checking || authLoading || connecting) return
    
    setConnecting(true)
    
    // Directly redirect to OAuth start endpoint
    const nextPath = `/${locale}/results`
    const oauthUrl = `/api/auth/instagram?provider=instagram&locale=${locale}&next=${encodeURIComponent(nextPath)}`
      ...
      <section className="w-full max-w-3xl mx-auto mt-6 rounded-2xl border border-white/10 bg-white/5 p-6">
        ...
        <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2">
          <Button
            ...
            aria-busy={checking || connecting ? true : undefined}
          ...
          >
            {connecting ? (
              <span className="inline-flex items-center gap-2">
                ...
                <span>{locale === "zh-TW" ? "連接中..." : "Connecting..."}</span>
              </span>
            ) : (
