...
function computeEngagementRatePct(input: {
  ...
  const explicit =
    typeof input.engagementRatePct === "number" && Number.isFinite(input.engagementRatePct)
  ...
  const comments =
    typeof input.avgComments === "number" && Number.isFinite(input.avgComments) ? input.avgComments : null
...
function buildDemoCreators({
  ...
  for (let i = 0; i < count; i++) {
    ...
    const erPct = clampNumber(1.2 + rand() * 6.2, 0.6, 9.9)
    const avgLikes = Math.round(((followers * erPct) / 100) * (0.82 + rand() * 0.22))
    ...
    out.push({
      id,
      displayName: locale === "zh-TW" ? "示範創作者" : "Demo creator",
      avatarUrl: svgAvatarDataUrl(id, locale === "zh-TW" ? "示範" : "Demo"),
      category: locale === "zh-TW" ? "創作者" : "Creator",
      followerCount: followers,
      avgLikes,
      avgComments,
      engagementRate: erPct / 100,
      isVerified: false,
      profileUrl: "#",
      isDemo: true,
    })
...
export function MatchmakingClient({ locale, initialCards }: MatchmakingClientProps) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const mapObj =
        parsed?.map && typeof parsed.map === "object" ? (parsed.map as Record<string, any>) : null
      if (!mapObj) return
      ...
      for (const [creatorId, v] of Object.entries(mapObj)) {
        ...
        const followers =
          typeof v?.followers === "number" && Number.isFinite(v.followers)
            ? Math.floor(v.followers)
            : undefined
        ...
        const didChange =
          prev?.followers !== next.followers || prev?.engagementRatePct !== next.engagementRatePct
  ...
  useEffect(() => {
    ...
    const nextCollab = (searchParams.get("collab") ?? "any") as CollabType | "any"
    ...
    if (nextBudget !== "custom") setCustomBudget("")
  ...
  const cardsWithDemos = useMemo(() => {
    ...
    const demos =
      missing > 0 ? buildDemoCreators({ locale, existingIds, count: missing, seedBase: "matchmaking" }) : []
  ...
  const creators: CreatorWithId[] = useMemo(() => {
    ...
    return cardsWithDemos.map((c) => {
      ...
      const handle = rawHandle && !/\s/.test(rawHandle) ? rawHandle : undefined
      ...
      const erPctFromCard = (() => {
        const v = (c as any)?.engagementRate
        if (typeof v !== "number" || !Number.isFinite(v)) return undefined
        const asPct = v > 1 ? v : v * 100
        return clampNumber(roundTo2(asPct), 0, 99)
      })()

      const computedErPct = computeEngagementRatePct({
        engagementRatePct:
          typeof (c as any)?.stats?.engagementRatePct === "number" && Number.isFinite((c as any).stats.engagementRatePct)
            ...
            : erPctFromCard,
        followers: typeof rawFollowers === "number" ? rawFollowers : null,
        avgLikes: typeof (c as any).avgLikes === "number" ? (c as any).avgLikes : null,
        avgComments: typeof (c as any).avgComments === "number" ? (c as any).avgComments : null,
      })

      const rawErPct =
        typeof cachedStats?.engagementRatePct === "number" && Number.isFinite(cachedStats.engagementRatePct)
          ? cachedStats.engagementRatePct
          : typeof computedErPct === "number" && Number.isFinite(computedErPct)
            ? computedErPct
            : undefined

      const avatarUrl = typeof c.avatarUrl === "string" && c.avatarUrl.trim() ? c.avatarUrl.trim() : undefined
      const finalAvatarUrl = avatarUrl ? avatarUrl : svgAvatarDataUrl(String(c.id), c.displayName)
      ...
      return {
        ...
        deliverables,
        minPrice: typeof c.minPrice === "number" && Number.isFinite(c.minPrice) ? c.minPrice : undefined,
        stats: {
          followers: typeof rawFollowers === "number" ? rawFollowers : undefined,
          engagementRate: typeof rawErPct === "number" ? rawErPct / 100 : undefined,
        },
        contact: typeof c.contact === "string" ? c.contact : null,
  ...
  useEffect(() => {
    ...
    const missing = visibleCreatorIds.filter((id) => !statsCacheRef.current.has(id) && !statsInFlightRef.current.has(id))
    ...
    const fetchOne = async (creatorId: string) => {
      ...
      const engagementRatePct = typeof stats?.engagementRatePct === "number" && Number.isFinite(stats.engagementRatePct) ? clampNumber(roundTo2(stats.engagementRatePct), 0, 99) : null
      ...
      const changed = prevCached?.followers !== nextCached.followers || prevCached?.engagementRatePct !== nextCached.engagementRatePct
  ...
  }, [canRenderMatchmaking, visibleCreatorIdsKey, visibleCreatorIds])
    ...
    persistTimerRef.current = setTimeout(() => {
      try {
        ...
        for (const [creatorId, v] of statsCacheRef.current.entries()) {
          ...
          const engagementRatePct = typeof v?.engagementRatePct === "number" && Number.isFinite(v.engagementRatePct) ? clampNumber(roundTo2(v.engagementRatePct), 0, 99) : undefined
          if (followers == null && engagementRatePct == null) continue
        ...
        {!authChecked ? (
          ...
          <>
            ...
            <div className="w-full max-w-[1200px] mx-auto px-3 sm:px-6 mt-4">
              <div className="text-xs sm:text-sm text-white/70 min-w-0 truncate">
                {uiCopy.matchmaking.recommendedLabel}
              </div>
            </div>
